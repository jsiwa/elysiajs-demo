...
// 管理员权限检查函数
function requireAdmin({ user, set, request }: any) {
  if (!user) {
    // 未登录，重定向到登录页面
...
export const adminRoutes = new Elysia({ prefix: '' })
  ...
  .use(createI18nRoutes(new Elysia(), [
    {
      path: '/admin/files',
      handler: async ({ lang, t, currentPath, user, query, set, request }) => {
        // 检查管理员权限
        const authResult = requireAdmin({ user, set, request })
        if (authResult) return authResult
        ...
        try {
          ...
          return FileManagerPage({ 
            ...
            t, 
            files, 
            currentFolder: query.folder as string || '', 
            currentPath, 
            user 
          })
        } catch (error) {
